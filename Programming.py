...
# Чтобы сравнить свой код с правильным кодом, нажать на консоли ПКМ -> Compare with Clipboard,
# в левом окошке вставить свой код.
...
'''
# ЧИСЛА (** возводит в степень; // делит без остатка; % выводит остаток)
x = 5
# Согласно PEP 8 если используются операторы с разными приоритетами, 
# то нужно поставить пробелы только вокруг операторов с наименьшим приоритетом (ами):
...
print(a)  # [1, 'd', 'g', 4, [3, 1], 6, 'd']
del a[0]
print(a)  # ['d', 'g', 4, [3, 1], 6, 'd']
a.remove('d')  # удаляет первое вхождение элемента, если элемента нет, то ошибка
print(a)  # ['g', 4, [3, 1], 6, 'd']
...
days = [(i, d[i % 7]) for i in range(1, 78)]  # бесконечный проход по списку
for i in days:
    print(i)
'''
...
print(a, b, c)  # 1 2 3
q = w = [1, 2, 3]  # массовое присвоение (для изменяемых объектов так не делать)
...
# поэтому лучше принять себе за правило: при описании декоратора, все принимаемые аргументы,
...
@timer(iters=1000)  # аналогично result = timer(1000)(test)(10000)
def test(n):
...
# наследование, магический метод __new__()
class Animal:
...
'''
# ИМПОРТ МОДУЛЯ (другого файла, написанного в python и не только)
import heat  # при импорте модуля, выполняется код этого модуля
